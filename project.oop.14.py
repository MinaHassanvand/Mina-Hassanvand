...
class Car(Vehicle):
    ...
    def feeCalc(self,myInputs):
        if self.myInputs['durationRounded'] >= 0 and self.myInputs['durationRounded']  <= 3: 
...
class Truck(Vehicle):
    ...
    def feeCalc(self, myInputs):
        if self.myInputs['durationRounded']  >= 0 and self.myInputs['durationRounded']  <= 2: 
...
class Bus(Vehicle):
    ...
    def feeCalc(self, myInputs):
        if self.myInputs['durationRounded']  <= 1.00: 
...
def instanceMaker():
    if myInputs['carType'] == 1:
        instanceCar = Car()
        instanceCar.feeCalc(myInputs)
    
    if myInputs['carType'] == 2:
        instanceTruck = Truck()
        instanceTruck.feeCalc(myInputs)

    if myInputs['carType']== 3:
        instanceBus = Bus()
        instanceBus.feeCalc(myInputs)
...
def parkingInfoInputCarType():
    inputCarType = 0
    flag = True
    while flag:
        try:
            inputCarType = int(input('Car types? 1 for car, 2 for bus, or 3 for truck. Press 99 to stop: '))           
            if inputCarType in carTypeCode:
                myInputs['carType'] = inputCarType
                break
        except:
        ...
        if inputCarType == 99:
            raise SystemExit
    return myInputs, inputCarType

while True:

    if parkingInfoInputCarType():
        instanceCarType = Vehicle(myInputs)
        instanceCarType.parkingInfoInputHourIn()
        instanceCarType.parkingInfoInputMinIn()
        instanceCarType.parkingInfoInputHourOut(hourIn)
        instanceCarType.parkingInfoInputMinOut()
        instanceCarType.parkingDurationCalc()
        
        instanceForChargeCalc = instanceMaker()
        
        instanceCarType.parkingBill()
        instanceCarType.parkingLog()
    else:
        raise SystemExit
#-------------------------------------------------------------------------------------------------------------
