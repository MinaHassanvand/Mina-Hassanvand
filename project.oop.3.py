...
class Car(Vehicle):
    ...
    def __init__(self, myInputs):
        super.__init__(self, myInputs)
    
    def feeCalc():

        if self.myInputs['durationRounded'] >= 0 and self.myInputs['durationRounded']  <= 3 and self.myInputs['carType'].lower() == self.myCarTypes[0]: # calculates car fee
            self.myInputs['charge'] = 0
            
        elif self.myInputs['durationRounded']  > 3 and self.myInputs['carType'].lower() == self.myCarTypes[0]:
...
class Truck(Vehicle):
    ...
    def feeCalc():
        ...
        if self.myInputs['durationRounded']  >= 0 and self.myInputs['durationRounded']  <= 2 and self.myInputs['carType'].lower() == self.myCarTypes[1]: # calculates truck fee
            self.myInputs['charge'] = self.myInputs['durationRounded'] * 1   
            
        elif self.myInputs['durationRounded']  > 2 and self.myInputs['carType'].lower() == self.myCarTypes[1]:
...
class Bus(Vehicle):
    ...
    def feeCalc():
        ...
        if self.myInputs['durationRounded']  <= 1.00  and self.myInputs['carType'].lower() == self.myCarTypes[2]: # calculates bus fee
            self.myInputs['charge'] = 2
        
        elif self.myInputs['durationRounded']  > 1.00 and self.myInputs['carType'].lower() == self.myCarTypes[2]:
...
def instanceMaker(carType):
    if carType == 1:
        instance = Car(carType)
        return instance.feeCalc(carType)

    elif carType == 2:
        instance = Truck(carType)
        return instance.feeCalc(carType)
    
    elif carType== 3:
        instance = Bus(carType)
        return instance.feeCalc(carType)
...
while True:
    ...
    if parkingInfoInputCarType(myInputs):
        ...
        carType.parkingDurationCalc()
        
        instanceMaker(carType)
